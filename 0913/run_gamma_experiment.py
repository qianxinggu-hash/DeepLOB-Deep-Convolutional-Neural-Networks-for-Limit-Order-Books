#!/usr/bin/env python3
"""Gamma sensitivity for the paper-form GP mean/quadratic QVI method only.

Every gamma retains limit and inventory-reducing market controls. No other
parameter changes. Table-6-style halving grid plus seven earlier anchors.
"""
...
def validate(tests: list[dict], totals: dict, config: gp.GPConfig) -> dict:
    ...
    checks = {
        ...
        "inventory_within_cap": all(r["max_abs_inventory_lots"] <= config.max_inventory_lots for r in rows),
...
def main() -> None:
    ...
    result = {"as_of": datetime.now().astimezone().isoformat(), "instrument": "HK 07709", "period": "2026-07",
        ...
        "provenance": {"model": "0906/gp_qvi_model.py", "model_sha256": hashlib.sha256((ROOT / "0906/gp_qvi_model.py").read_bytes()).hexdigest(),
            "diagnostic_replay": "0913/paper_gp_replay.py", "diagnostic_replay_sha256": hashlib.sha256((HERE / "paper_gp_replay.py").read_bytes()).hexdigest(),
    ...
    anchor_path = HERE / "output/reference_anchor_results.json"
    if anchor_path.exists():
        for key, reference in json.loads(anchor_path.read_text()).items():
            if key in result["variants"]:
                checks[f"original_anchor_matches_{key}"] = reconcile(result["variants"][key]["tests"], reference)
