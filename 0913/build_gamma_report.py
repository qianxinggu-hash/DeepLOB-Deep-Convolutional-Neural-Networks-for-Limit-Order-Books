#!/usr/bin/env python3
"""Report the paper-form GP gamma study; no strategy or policy-constraint sweep."""
...
def main() -> None:
    ...
    sources = [
        {"id": "replay", "label": "GP-QVI gamma 单变量回测", "path": "0913/output/report_datasets.sqlite",
         "description": "Python展开已核对的原方法回放结果后，通过实际执行的SQLite查询读取报告数据。",
         "query": {"engine": "sqlite", "language": "sql", "sql": sql,
                   "tables_used": ["report_datasets", "0913/output/report_datasets.sqlite",
                                   "0913/output/gamma_results.json"],
        ...
        {"id": "model", "label": "0906 GP-QVI 模型及库存单位",
         ...
         "description": "保留原求解器、100股一手、gamma每手平方每秒、双侧限价及库存减仓市价控制。"},
    ]
    charts = []
    ...
    table("inventory", "库存暴露与市价调整明细", "through", [
        ("gamma_label", "gamma"), ("inventory_rms", "库存RMS手"), ("peak_inventory", "峰值手"),
        ("nonzero_inventory_time_pct", "持仓时间%"),
    ...
    prose("scope", "比较范围与指标定义", (
        ...
        "固定±10手库存上限、300秒规划期限、100步以及5ms/t+1…t+20执行规则。"))
    ...
    prose("next", "下一步围绕论文机制验证gamma，而非换策略", (
        "1. 根据可接受的时间加权库存、库存峰值和交易量预先规定gamma比较范围，再用测试日之前的数据选择参数。\n"
    ...
    prose("caveats", "工程复现与原论文的边界", (
        ...
        "- 保留日级合法tick识别、原成交代理及费用；不包含虚拟订单排队、隐藏流动性、最低佣金、借券和市场冲击。\n"
        "- gamma=5与原0906逐日一致，七个原档位的成交、库存峰值和现金损益均逐日核对；"
        "新增暴露计时不改变现金流，全部gamma通过库存积分、生命周期、费用与归零检查。\n"
    ...
    notes = {"audience": "product stakeholders", "delivery": "portable html",
        "structure": ["Title", "Executive Summary", "definitions", "paper mechanism", "gamma trade/risk/policy evidence",
                      "next steps", "further questions", "caveats"],
        "chart_map": {"fills": "bar, anchors only for legibility; full 21 rows in table",
                      "inventory": "ordered line, time-weighted inventory RMS",
                      "impulse": "bar, full-horizon equal-weight state surface fraction",
                      "frontier": "scatter, empirical daily net std vs daily mean, not MC IR/NIR"},
